...
| `country`            | str (enum/code) | ✅ Intake (edit: requesters) + Central (edit: product; read-only for others)          | ✅  | ❌        | e.g. `UK`, `AE`, `BR` |
| `market`             | str (enum/code) | ✅ Central only (edit: product; read-only for others)                                 | ✅  | ❌        | Region/market classification maintained outside intake |
...
| `hypothesis`        | str  | ✅ Central only (edit: product; read-only for others; can be LLM-suggested)             | ✅  | ❌        | Explicit cause-effect hypothesis owned downstream from intake |
...
| `customer_segment`               | str (enum)  | ✅ Central only (edit: product; read-only for others)                                                     | ✅                                  | ❌        | e.g. `SMB`, `Enterprise`, `Consumer`                                                          |
| `initiative_type`                | str (enum)  | ✅ Central only (edit: product; read-only for others)                                                     | ✅                                  | ❌        | e.g. `Feature`, `Experiment`, `TechDebt`, `OpsImprovement`                                    |
...
| Field                     | Type       | Sheet                                                                                                    | DB | Derived? | Notes                                                                |
| ------------------------- | ---------- | -------------------------------------------------------------------------------------------------------- | -- | -------- | -------------------------------------------------------------------- |
| `effort_tshirt_size`      | str (enum) | ✅ Central only (edit: product/eng; read-only for others)                                                  | ✅  | ❌        | `XS`, `S`, `M`, `L`, `XL`                                            |
| `effort_engineering_days` | float      | ✅ Central only (edit: product/eng; read-only for others)                                                  | ✅  | ❌        | If engineering provides a direct estimate                            |
| `effort_other_teams_days` | float      | ✅ Central only (edit: product; read-only for others)                                                      | ✅  | ❌        | Design, Ops, CS, etc.                                                |
| `infra_cost_estimate`     | float      | ✅ Central only (edit: product/finance; read-only for others)                                              | ✅  | ❌        | One-off infra/platform cost estimate if any                          |
...
| `dependencies_others`      | str         | ✅ Central only (edit: product; read-only for others)                                | ✅                             | ❌        | Free-text dependencies on systems, vendors, legal, etc.                                                       |
| `risk_level`               | str (enum)  | ✅ Central only (edit: product; read-only for others)                                | ✅                             | ❌        | `Low`, `Medium`, `High`                                                                                       |
| `risk_description`         | str         | ✅ Central only (edit: product; read-only for others)                                | ✅                             | ❌        | Why risk level is high/medium/low                                                                             |
| `time_sensitivity_score`   | float       | ✅ Central only (edit: product; read-only for others)                                | ✅                             | ❌        | Numeric urgency input used in scoring and prioritization                                                      |
