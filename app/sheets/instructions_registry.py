...
INSTRUCTIONS_REGISTRY: Dict[Tuple[SheetType, TabName], TabInstructions] = {
    ...
    ("product_ops", "mathmodels"): TabInstructions(
        ...
        lines=(
            ...
            "LLM suggestions use the selected MathModels row, initiative context from DB, shared context from LLM_Context, and active KPI definitions from Metrics_Config.",
        ...
        steps=(
            ...
            "4. OPTION B — LLM suggestion: Optionally update LLM_Context with shared business assumptions, then run 'Suggest math model (LLM)' from menu.",
            "5. Review LLM columns (read-only): 'llm_suggested_formula_text' (M), 'llm_suggested_metric_chain_text' (I), 'llm_notes' (N).",
            "6. The suggestion also uses active KPI definitions from Metrics_Config, so keep KPI keys and descriptions there current.",
            "7. If LLM suggestion is good: copy to 'formula_text' (J), 'metric_chain_text' (F), 'assumptions_text' (O). PM input → DB.",
            "8. If the suggestion is weak, improve row context or LLM_Context and rerun instead of editing LLM-owned columns directly.",
            "9. Set 'approved_by_user' (L) = TRUE to approve formula. PM input → DB.",
            "10. Select rows → run 'Save selected rows' from menu to persist to DB.",
            "11. Run 'Seed math params' from menu — extracts variables from formula → creates rows in Params tab.",
            "12. Go to Params tab → fill parameter values → Save → return to Scoring_Inputs → run 'Score selected initiatives'.",
            "13. COMPUTED SCORE WRITEBACK: After 'Score selected initiatives' runs with MATH_MODEL framework:",
        ...
        warnings=(
            ...
            "pm.suggest_math_model_llm does not auto-sync sheet edits into the DB first; save the relevant source rows before relying on fresh DB context.",
    ...
    ("central_backlog", "backlog"): TabInstructions(
        ...
        lines=(
            ...
            "Ownership-aware sync is available: PM edits flow to DB via pm.save_selected, and pm.sync_backlog_db reconciles PM-owned sheet fields with DB-owned fields safely.",
        ...
        steps=(
            ...
            "4. Edit strategic PM-owned fields: Hypothesis, Problem Statement, and Strategic Priority Coefficient.",
            ...
            "11. Select rows → run 'Sync DB ↔ Backlog (safe)' to reconcile PM-owned edits with DB-owned fields and inspect the per-row diff dialog.",
            "12. Select rows → run 'Generate LLM Summary' when you want the backend to overwrite LLM Summary from structured DB + Backlog Description context.",
            "13. Run 'Switch scoring framework' to activate a different scoring framework's scores.",
            "DATA FLOWS:",
            ...
            "- 'Sync DB ↔ Backlog (safe)' (pm.sync_backlog_db): Sheet ↔ DB. PM-owned fields flow into DB; DB-owned fields flow back to sheet; UI shows row-level diffs after success.",
            "- 'Generate LLM Summary' (pm.generate_llm_summary): DB + Backlog Description → DB + this sheet. Rebuilds backend-owned LLM Summary for selected rows.",
        ...
        warnings=(
            ...
            "'LLM Summary' is backend-owned now — editing the cell or running 'Save selected rows' will not persist a manual override to DB.",
