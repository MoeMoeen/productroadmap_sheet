...
INSTRUCTIONS_REGISTRY: Dict[Tuple[SheetType, TabName], TabInstructions] = {
    ...
    ("product_ops", "scoring_inputs"): TabInstructions(
        ...
        steps=(
            ...
            "6. Select rows → run 'Save selected rows' from menu to persist inputs to DB.",
            "7. Run 'Score selected initiatives' from menu — backend computes all frameworks, writes to score columns.",
            "8. Review score columns (read-only): active_*_score (F-H), rice_*_score (U-W), wsjf_*_score (X-Z), math_*_score (I-K).",
            "9. Optionally run 'Switch scoring framework' from menu to change which framework's scores are active.",
            "10. For Math Model scoring: first complete MathModels tab → Params tab workflow, then return here and run 'Score selected initiatives'.",
            "11. KPI CONTRIBUTIONS: When scoring with MATH_MODEL framework, 'Score selected initiatives' also computes KPI contributions:",
    ...
    ("central_backlog", "backlog"): TabInstructions(
        ...
        lines=(
            ...
            "PMs can freely add formula columns and reorder columns — backend matches by header name.",
        ),
        steps=(
            "1. Run 'Sync intake → backlog' from Roadmap AI menu to sync all intake sheets → DB → this sheet.",
            "2. Review initiative data: core fields, scores, dependencies, candidacy status.",
            ...
            "DATA FLOWS:",
            "- 'Sync intake → backlog' (pm.backlog_sync): DB → Sheet. Overwrites ALL columns from DB including scores.",
        ...
        warnings=(
            ...
            "Formula columns (e.g., Metric Chain JSON, engineering_tokens, deadline_date, is_mandatory) are PM-managed — backend ignores them.",
