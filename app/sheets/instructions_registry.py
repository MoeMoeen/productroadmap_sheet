...
INSTRUCTIONS_REGISTRY: Dict[Tuple[SheetType, TabName], TabInstructions] = {
    ...
    ("optimization_center", "candidates"): TabInstructions(
        ...
        lines=(
            ...
            "KPI contribution columns are display-only — enter values in ProductOps/KPI_Contributions.",
        ),
        steps=(
            "1. Verify Settings tab has correct scenario_name and constraint_set_name.",
            "2. Run 'Populate Candidates' from menu — pulls constraint data from Constraints tab into read-only columns (L-Q).",
            ...
            "4. Review KPI contributions (J-K) — display-only, sourced from ProductOps/KPI_Contributions tab.",
        ...
        warnings=(
            ...
            "KPI contributions (J-K) display-only — edit in ProductOps/KPI_Contributions tab.",
        ),
    ),
    ...
    ("optimization_center", "targets"): TabInstructions(
        ...
        steps=(
            ...
            "   - 'target_value' (G): numeric threshold. PM input → DB.",
            "3. Add 'notes' (H) for documentation. PM input → DB.",
            "4. Optionally run 'Save' from menu to validate and persist targets to DB immediately.",
            "5. Note: Optimization auto-syncs — when you run 'Optimize Selected/All' from Candidates, backend reads Constraints + Targets tabs and syncs to DB before solver runs (no manual Save required).",
            "6. Go to Candidates tab → run optimization.",
            "7. After run: check Gaps_and_Alerts tab to see if targets were met or missed.",
        ),
        actions=("pm.save_optimization",),
        warnings=(
    ...
    ("product_ops", "scoring_inputs"): TabInstructions(
        ...
        lines=(
            "Primary scoring surface. Fill framework inputs, run scoring, compare results.",
            "initiative_key (A) = formula from Central Backlog — read-only.",
            "Framework inputs (RICE: M-P, WSJF: Q-T) = PM input → DB.",
            "Score columns (F-L, U-Z) = Backend writes — read-only.",
        ),
        steps=(
            "1. Review formula column (auto-populated, read-only): initiative_key (A) from Central Backlog.",
            "2. Set 'active_scoring_framework' (C): 'RICE', 'WSJF', or 'MATH_MODEL'. PM input → DB.",
            "3. Set 'use_math_model' (D) = TRUE if using math model scoring. PM input → DB.",
            "4. Fill RICE inputs (M-P): rice_reach, rice_impact, rice_confidence, rice_effort. PM input → DB.",
            "5. Fill WSJF inputs (Q-T): wsjf_business_value, wsjf_time_criticality, wsjf_risk_reduction, wsjf_job_size. PM input → DB.",
            "6. Select rows → run 'Save Selected' from menu to persist inputs to DB.",
            "7. Run 'Score Selected' from menu — backend computes all frameworks, writes to score columns.",
            "8. Review score columns (read-only): active_*_score (F-H), rice_*_score (U-W), wsjf_*_score (X-Z), math_*_score (I-K).",
            "9. Optionally run 'Switch Framework' from menu to change which framework's scores are active.",
            "10. For Math Model scoring: first complete MathModels tab → Params tab workflow, then return here and run 'Score Selected'.",
            "11. KPI CONTRIBUTIONS: When scoring with MATH_MODEL framework, 'Score Selected' also computes KPI contributions:",
        ...
        warnings=(
            "Column A (initiative_key) is formula — do not edit.",
    ...
    ("product_ops", "mathmodels"): TabInstructions(
        ...
        lines=(
            ...
            "computed_score = Backend writes — auto-populated after 'Score Selected' runs on Scoring_Inputs.",
        ),
        steps=(
            "1. Review formula column (auto-populated, read-only): initiative_key (A) from Central Backlog.",
            ...
            "4. OPTION B — LLM suggestion: Run 'Suggest Math Model' from menu.",
            ...
            "8. Select rows → run 'Save Selected' from menu to persist to DB.",
            "9. Run 'Seed Math Params' from menu — extracts variables from formula → creates rows in Params tab.",
            "10. Go to Params tab → fill parameter values → Save → return to Scoring_Inputs → run 'Score Selected'.",
            "11. COMPUTED SCORE WRITEBACK: After 'Score Selected' runs with MATH_MODEL framework:",
            ...
            "    - Primary model's score (is_primary=TRUE) is displayed in Scoring_Inputs as active_math_model_score.",
        ),
    ...
    ("product_ops", "params"): TabInstructions(
        ...
        steps=(
            ...
            "4. Select rows → run 'Save Selected' from menu to persist to DB.",
            "5. Go to Scoring_Inputs tab → run 'Score Selected' to compute formula with your values.",
        ),
    ...
    ("product_ops", "kpi_contributions"): TabInstructions(
        ...
        steps=(
            "1. HOW THIS TAB GETS POPULATED: Run 'Score Selected' on Scoring_Inputs tab with MATH_MODEL framework.",
            ...
            "6. Select rows → run 'Save Selected' from menu to persist to DB.",
